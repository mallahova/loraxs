...
def main():
    ...
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, TrainingArguments,RankDropoutArguments)
    )
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args,rank_dropout_args = parser.parse_args_into_dataclasses()
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for k in [12, 16, 20, 25]:
            mcc_k_results[f"mcc_{k}"] = compute_mcc_for_k(k, model_args.lora_rank)
            print(mcc_k_results[f"mcc_{k}"])

        result.update(mcc_k_results)
        print(result)
        return result
    
    import math
    def compute_mcc_for_k(k: int, rank: int) -> float:
        """
        Computes the Matthew's Correlation Coefficient (MCC) for a given k by masking the parameters
        ...
        Returns:
            float: The computed MCC for the given k.
        """
        print(f"Computing MCC for k={k}:", end=" ")
        params_copy = {}
        with torch.no_grad():
